...
```
### Filtering
The data was filtered using a threshold value of 3. Transcripts that do not have intensities larger than the threshold in at least as many arrays as the smallest experimental group are excluded.
```{r}
par(mfrow=c(2,1))
...
### Model Fit
*voom* function adjusts the data for heteroscedasticity and prepares the data for linear fit. The design and contrast matrices were fit in a linear model using the corresponding function from the limma package. Empirical Bayes method was used to make more precise estimates of gene-wise variability and to get moderated results.
```{r}
par(mfrow=c(1,2))
...
# Results
Significance of the differential expression of of the genes were determined using the function topTable from the limma library. 
```{r}
